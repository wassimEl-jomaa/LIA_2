...
class Project(Base):
    ...
    # NOTE: `organization_id` is used as the FK; do not map a separate `organization` column
    # here because the database schema does not include `projects.organization`.
    ...
    organization_rel = relationship("Organization", back_populates="projects")  # avoid name clash

    logs = relationship("RequestLog", back_populates="project", cascade="all, delete-orphan")
...
class TestCase(Base):
    ...
    # Optional link to requirement (user story)
    ...
    # store steps/expected as text (you can later normalize into separate tables if needed)
...
class TestExecution(Base):
    ...
    __table_args__ = (
        UniqueConstraint("test_case_id", "created_at", name="uq_test_executions_case_created_at"),
    )
...
class RequestLog(Base):
    ...
    # Note: some DB schemas may not include a `test_execution_id` FK on request_logs.
    # This model intentionally does not assume a `test_execution_id` column exists
    # in the database; therefore there is no relationship to `TestExecution` here.
